...
async function boot($: any, on: any, o: Opts = {}) {
  ...
  on('prompt.submit', ($: any, e: any) => ({ text: `bottom:${e.text}` }))
...
const say = ($: any, text: string, kind = 'composer') => $.prompt.submit({ text, origin: { kind } } as any)

describe('prompt 意圖（經由 prompt.submit）', () => {
  test('使用者的 prompt 原樣放行；「幫我修登入閃退」→ 修bug 推測 · 重現', async ($, on) => {
    await boot($, on)
    const r: any = await say($, '幫我修登入閃退')
  ...
  test('已有 fresh 任務：prompt 關鍵字不換任務', async ($, on) => {
    await boot($, on)
    await $.tool.call({ tool: TOOL, task: 'feature', stage: 'impl' } as any)
    await say($, '修一下這個 typo')
    const rows = await band($)
    expect(rows[0]).toMatch(/^新功能 ●/)
    expect(rows[1]?.trim()).toMatch(/^實作 · /)
  })
  test('chat 意圖：送出當下就顯示問答（有 fresh 任務時第三行「討論中」）', async ($, on) => {
    await boot($, on)
    await turnStart($, 't1')
    await $.tool.call({ tool: TOOL, task: 'feature', stage: 'impl' } as any)
    await turnComplete($, 't1')
    expect((await band($)).join('\n')).not.toContain('討論中')
    await say($, '這個會很消耗token嗎？')
    expect((await band($))[2]).toBe('討論中')
  })
  test('chat 意圖：上個 turn 有動作、任務未判定 → 送出後改畫「問答中 · 上次…」', async ($, on) => {
    await boot($, on, { store: { [`stage:${CWD}`]: { task: 'feature', stage: 'ship', stageSince: T0, updatedAt: T0 - 17 * 60 * MIN, source: 'guess', sessionId: 'old' } } })
    await turnStart($, 't1')
    await $.tool.call({ tool: 'Bash', command: 'npm run build' } as any)
    await turnComplete($, 't1')
    expect((await band($))[0]).toBe('上次：新功能 · TF · 17 小時前')
    await say($, '為何 bundle 這麼久？')
    expect((await band($))[0]).toBe('問答中 · 上次：新功能 · TF · 17 小時前')
  })
  test('N1：要求動手但本 session 宣告的任務已過期（prompt 不取代）→ 不改成「問答中」', async ($, on) => {
    const { clock } = await boot($, on)
    await turnStart($, 't1')
    await $.tool.call({ tool: TOOL, task: 'feature', stage: 'impl' } as any)
    await turnComplete($, 't1')
    await clock.advance(121 * MIN)
    expect((await band($))[0]).toBe('上次：新功能 · 實作 · 2 小時前')
    await say($, '幫我修好登入')
    expect((await band($))[0]).toBe('上次：新功能 · 實作 · 2 小時前')
  })
  test('N2：turn 進行中打的 prompt 排成自己的 turn：那個 turn 開始時才套用', async ($, on) => {
    await boot($, on)
    await turnStart($, 't1')
    await $.tool.call({ tool: 'Bash', command: 'npm run build' } as any)
    await $.prompt.submit({ text: '幫我修登入閃退', origin: { kind: 'composer' }, turnId: 't1' } as any)
    expect((await band($))[0]).toBe('判斷任務中…')
    await turnComplete($, 't1')
    expect((await band($))[0]).toBe('問答中')
    await $.turn.start({ text: '幫我修登入閃退', turnId: 't2' } as any)
    const rows = await band($)
    expect(rows[0]).toMatch(/^修bug 推測 ◉/)
  })
  test('N2：排隊的問句在自己的 turn 開始時才算問答', async ($, on) => {
    await boot($, on)
    await turnStart($, 't1')
    await $.tool.call({ tool: TOOL, task: 'feature', stage: 'impl' } as any)
    await $.prompt.submit({ text: '還要多久？', origin: { kind: 'composer' }, turnId: 't1' } as any)
    await turnComplete($, 't1')
    expect((await band($)).join('\n')).not.toContain('討論中')
    await $.turn.start({ text: '還要多久？', turnId: 't2' } as any)
    expect((await band($))[2]).toBe('討論中')
  })
  test('N2：排隊的 prompt 若先開始的是別的 turn 就丟掉', async ($, on) => {
    await boot($, on)
    await turnStart($, 't1')
    await $.tool.call({ tool: 'Bash', command: 'npm test' } as any)
    await $.prompt.submit({ text: '幫我修登入閃退', origin: { kind: 'composer' }, turnId: 't1' } as any)
    await turnComplete($, 't1')
    await $.turn.start({ text: '別的 prompt', turnId: 't2' } as any)
    await $.turn.start({ text: '幫我修登入閃退', turnId: 't3' } as any)
    expect((await band($))[0]).toBe('問答中')
  })
  test('/clear 後的第一句 prompt：turn.start 換了 session id 也仍是現在的任務', async ($, on) => {
    const { clearSession } = await boot($, on, { ids: ['sess-1', 'sess-2'] })
    clearSession()
    await say($, '幫我修登入閃退')
    await $.turn.start({ text: '幫我修登入閃退', turnId: 't1' } as any)
    expect((await band($))[0]).toMatch(/^修bug 推測 ◉/)
  })
  test('N6：實際順序 say → turn.start（同一段文字）只套用一次', async ($, on) => {
    await boot($, on)
    await say($, '幫我修登入閃退')
    await $.turn.start({ text: '幫我修登入閃退', turnId: 't1' } as any)
    let rows = await band($)
    ...
    await turnComplete($, 't1')
    await say($, '這個會很消耗token嗎？')
    await $.turn.start({ text: '這個會很消耗token嗎？', turnId: 't2' } as any)
    rows = await band($)
    expect(rows[0]).toMatch(/^修bug 推測 ◉/)
    expect(rows[2]).toBe('討論中')
  })
  test('turn 進行中插話的問句不抹掉這個 turn 的動作', async ($, on) => {
    await boot($, on)
    await turnStart($, 't1')
    await $.tool.call({ tool: 'Bash', command: 'npm run build' } as any)
    await $.prompt.submit({ text: '還要多久？', origin: { kind: 'composer' }, turnId: 't1' } as any)
  ...
  test('要求動手的 prompt 清掉上一輪的「討論中」', async ($, on) => {
    await boot($, on)
    await turnStart($, 't1')
    await $.tool.call({ tool: TOOL, task: 'feature', stage: 'impl' } as any)
    await turnComplete($, 't1')
  ...
  test('prompt 推測 bugfix 後第一次 Edit 程式檔仍是 bugfix', async ($, on) => {
    await boot($, on)
    await say($, '幫我修登入閃退')
    await $.tool.call({ tool: 'Edit', file_path: '/p/src/Login.swift' } as any)
    expect((await band($))[0]).toMatch(/^修bug 推測 /)
  })
  test('slash command 與接續語不判斷', async ($, on) => {
    await boot($, on)
    await say($, '/ios-review 修 bug')
    await say($, 'push')
    expect((await band($))[0]).toBe('問答中')
  })
})
